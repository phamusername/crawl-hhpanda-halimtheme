<?php
add_action('admin_menu', 'hhpanda_tool_add_menu');
function hhpanda_tool_add_menu()
{
	add_menu_page(
		'Thông Tin Tools',
		'Crawl HHPANDA.tv',
...
function hhpanda_crawl_tools()
{

	$cache = new HaLim\Cache();

	$categoryFromApi = $cache->readCache(HHPANDA_API_DOMAIN . "/the-loai");
	if(!$categoryFromApi) { // Kiểm tra nếu cache tồn tại
		$categoryFromApi = file_get_contents(HHPANDA_API_DOMAIN . "/the-loai");
		$cache->timeCache = 86400;
		$cache->saveCache(HHPANDA_API_DOMAIN . "/the-loai", $categoryFromApi); // Lưu cache
	}
	$categoryFromApi = json_decode($categoryFromApi);

	$countryFromApi = $cache->readCache(HHPANDA_API_DOMAIN . "/quoc-gia");
	if(!$countryFromApi) { // Kiểm tra nếu cache tồn tại
		$countryFromApi = file_get_contents(HHPANDA_API_DOMAIN . "/quoc-gia");
		$cache->timeCache = 86400;
		$cache->saveCache(HHPANDA_API_DOMAIN . "/quoc-gia", $countryFromApi); // Lưu cache
	}
...
<div class="wrap">
    ...
    <a href="?page=crawl-hhpanda-tools" class="nav-tab <?php if($tab===null):?>nav-tab-active<?php endif; ?>">Thủ Công</a>
    <a href="?page=crawl-hhpanda-tools&tab=schedule" class="nav-tab <?php if($tab==='schedule'):?>nav-tab-active<?php endif; ?>">Tự Động</a>
    <a href="?page=crawl-hhpanda-tools&tab=about" class="nav-tab <?php if($tab==='about'):?>nav-tab-active<?php endif; ?>">Giới Thiệu</a>
  </nav>
	<div class="tab-content">
		<?php
			switch($tab) :
				case 'schedule':
				$crawl_hhpanda_settings = json_decode(get_option(CRAWL_HHPANDA_OPTION_SETTINGS, []));
				$schedule_log = hhpanda_getLastLog();
		?>
					...
					<div class="postbox">
						<div class="inside">
							...
							<div>
								...
								<p>
									Cấu hình CronJob: <code><i style="color:blueviolet">*/10 * * * *</i> cd <i style="color:blueviolet">/path/to/</i>wp-content/plugins/crawl_hhpanda_halimthemes/ && php -q schedule.php <i style="color:blueviolet">{khóa bảo mật}</i></code>
								</p>
								<p>
									Ví dụ:
									<br />
									Mỗi 5 phút: <code>*/5 * * * * cd <?php echo CRAWL_HHPANDA_PATH; ?> && php -q schedule.php <i style="color:blueviolet"><?php echo get_option(CRAWL_HHPANDA_OPTION_SECRET_KEY, ''); ?></i></code>
									<br />
									Mỗi 10 phút: <code>*/10 * * * * cd <?php echo CRAWL_HHPANDA_PATH; ?> && php -q schedule.php <i style="color:blueviolet"><?php echo get_option(CRAWL_HHPANDA_OPTION_SECRET_KEY, ''); ?></i></code>
								</p>
							</div>
						</div>
					</div>
				...
				<div class="crawl_page">
					<div class="postbox">
						<div class="inside">
							...
							<div>
								<p>
									Khóa bảo mật: <input type="text" name="crawl_hhpanda_schedule_secret" value="<?php echo get_option(CRAWL_HHPANDA_OPTION_SECRET_KEY, ''); ?>">
									<button id="save_crawl_hhpanda_schedule_secret" class="button">Lưu</button>
								</p>
							</div>
							<div>
								<p>
									Kích hoạt:
									<input type="checkbox" class="wppd-ui-toggle" id="crawl_hhpanda_schedule_enable" name="crawl_hhpanda_schedule_enable"
										value=""
										<?php echo (json_decode(file_get_contents(CRAWL_HHPANDA_PATH_SCHEDULE_JSON))->enable === true) ? 'checked' : ''; ?>
									>
								</p>
							</div>
							<div>
								<p>Trạng thái: <?php echo (int) get_option(CRAWL_HHPANDA_OPTION_RUNNING, 0) === 1 ? "<code style='color: green'>Bật</code>" : "<code style='color: red'>Tắt</code>"; ?></p>
							</div>
							<div>
								<p>Bỏ qua định dạng: <code style="color: red"><?php echo join(', ', $crawl_hhpanda_settings->filterType);?></code></p>
								<p>Bỏ qua thể loại: <code style="color: red"><?php echo join(', ', $crawl_hhpanda_settings->filterCategory);?></code></p>
								<p>Bỏ qua quốc gia: <code style="color: red"><?php echo join(', ', $crawl_hhpanda_settings->filterCountry);?></code></p>
							</div>
							<div>
								<p>Page đầu: <code style="color: blue"><?php echo $crawl_hhpanda_settings->pageFrom;?></code></p>
								<p>Page cuối: <code style="color: blue"><?php echo $crawl_hhpanda_settings->pageTo;?></code></p>
							</div>
		...
		?>
				<div class="crawl_page">
					<div class="postbox">
						<div class="inside">
							HHPANDATV.COM - Tài nguyên phim, hiệu quả lâu dài và vĩnh viễn, và miễn phí sử dụng mãi mãi. <br />
		...
		?>
			<div class="crawl_main">
				<div class="crawl_filter notice notice-info">
					...
					<p>
						<div id="save_crawl_hhpanda_schedule" class="button">Lưu cấu hình cho Crawl(cào) tự động</div>
					</p>
				</div>
...
add_action('wp_ajax_save_crawl_hhpanda_schedule_secret', 'save_crawl_hhpanda_schedule_secret');
function save_crawl_hhpanda_schedule_secret()
{
	update_option(CRAWL_HHPANDA_OPTION_SECRET_KEY, $_POST['secret_key']);
	die();
}

add_action('wp_ajax_crawl_hhpanda_schedule_enable', 'crawl_hhpanda_schedule_enable');
function crawl_hhpanda_schedule_enable()
{
	$schedule = array(
		'enable' => $_POST['enable'] === 'true' ? true : false
	);
	file_put_contents(CRAWL_HHPANDA_PATH_SCHEDULE_JSON, json_encode($schedule));
	die();
}

add_action('wp_ajax_crawl_hhpanda_save_settings', 'crawl_hhpanda_save_settings');
function crawl_hhpanda_save_settings()
{
	$data = array(
	...
	if (!get_option(CRAWL_HHPANDA_OPTION_SETTINGS)) {
		add_option(CRAWL_HHPANDA_OPTION_SETTINGS, json_encode($data));
	} else {
		update_option(CRAWL_HHPANDA_OPTION_SETTINGS, json_encode($data));
	}
	die();
}

add_action('wp_ajax_crawl_hhpanda_page', 'crawl_hhpanda_page');
function crawl_hhpanda_page()
{
	echo crawl_hhpanda_page_handle($_POST['url']);
	die();
}

function crawl_hhpanda_page_handle($url)
{
	...
	if(count($sourcePage->items) > 0) {
		foreach ($sourcePage->items as $key => $item) {
			array_push($listMovies, "https://sv.xemphimmoi.xyz/phim/{$item->slug}|{$item->_id}|{$item->modified->time}|{$item->name}|{$item->origin_name}|{$item->year}");
		}
		return join("\n", $listMovies);
	}
	return $listMovies;
...
add_action('wp_ajax_crawl_hhpanda_movies', 'crawl_hhpanda_movies');
function crawl_hhpanda_movies()
{
	$data_post 					= $_POST['url'];
	...
	$result = crawl_hhpanda_movies_handle($url, $hhpanda_id, $hhpanda_update_time, $filterType, $filterCategory, $filterCountry);
	echo $result;
	die();
}

function crawl_hhpanda_movies_handle($url, $hhpanda_id, $hhpanda_update_time, $filterType, $filterCategory, $filterCountry)
{
	try {
		$args = array(
			'post_type' => 'post',
		...
		if ($total > 0) { # Trường hợp đã có
			...
			if ($wp_query->have_posts()) : while ($wp_query->have_posts()) : $wp_query->the_post();
					...
					if($_halim_metabox_options["fetch_hhpanda_update_time"] == $hhpanda_update_time) { // Không có gì cần cập nhật
						$result = array(
							'status'   			=> true,
							'post_id' 			=> null,
							'list_episode' 	=> [],
							...
							'wait'					=> false,
							'schedule_code' => HHPANDA_SCHEDULE_CRAWLER_TYPE_NOTHING
						);
					...
					$data 				= hhpanda_create_data($sourcePage, $url, $hhpanda_id, $hhpanda_update_time);

					$status = hhpanda_getStatus($data['status']);
					...
					$_halim_metabox_options["fetch_hhpanda_update_time"] 	= $data['fetch_hhpanda_update_time'];
					...
					// Re-Update Episodes
					$list_episode = hhpanda_get_list_episode($sourcePage, $post->ID);
					$result = array(
						'status'				=> true,
						'post_id' 			=> $post->ID,
						'data'					=> $data,
						...
						'wait'					=> true,
						'schedule_code' => HHPANDA_SCHEDULE_CRAWLER_TYPE_UPDATE
					);
					wp_update_post($post);
					return json_encode($result);
				endwhile;
		...
		$sourcePage =  HALIMHelper::cURL($api_url);
		$sourcePage = json_decode($sourcePage, true);
		$data 			= hhpanda_create_data($sourcePage, $url, $hhpanda_id, $hhpanda_update_time, $filterType, $filterCategory, $filterCountry);
		if($data['crawl_filter']) {
			$result = array(
				...
				'msg' 					=> "Lọc bỏ qua",
				'wait'					=> false,
				'schedule_code' => HHPANDA_SCHEDULE_CRAWLER_TYPE_FILTER
			);
			return json_encode($result);
		}

		$post_id 		= hhpanda_add_posts($data);
		$list_episode = hhpanda_get_list_episode($sourcePage, $post_id);
		$result = array(
			'status'				=> true,
			'post_id' 			=> $post_id,
			'data'					=> $data,
			'list_episode' 	=> $list_episode,
			'wait'					=> true,
			'schedule_code' => HHPANDA_SCHEDULE_CRAWLER_TYPE_INSERT
		);
		return json_encode($result);
  } catch (Exception $e) {
		$result = array(
			...
			'msg' 					=> $e->getMessage(),
			'wait'					=> false,
			'schedule_code' => HHPANDA_SCHEDULE_CRAWLER_TYPE_ERROR
		);
		return json_encode($result);
  }
}

function hhpanda_create_data($sourcePage, $url, $hhpanda_id, $hhpanda_update_time, $filterType = [], $filterCategory = [], $filterCountry = []) {
	...
	$data = array(
		'crawl_filter'						=> false,
		'fetch_url' 							=> $url,
		'fetch_hhpanda_id' 					=> $hhpanda_id,
		'fetch_hhpanda_update_time' => $hhpanda_update_time,
...
function hhpanda_add_posts($data)
{
	...
	if($data['poster'] && $data['poster'] != "") {
		$res 								= hhpanda_save_images($data['poster'], $post_id, $data['title']);
		$poster_image_url 	= str_replace(get_site_url(), '', $res['url']);
	}
	hhpanda_save_images($data['thumbnail'], $post_id, $data['title'], true);
	...
	$post_meta_movies = array(
		...
		'fetch_hhpanda_id'						=> $data['fetch_hhpanda_id'],
		'fetch_hhpanda_update_time'		=> $data['fetch_hhpanda_update_time'],
...
function hhpanda_save_images($image_url, $post_id, $posttitle, $set_thumb = false)
{
	...
	hhpanda_insert_attachment($res['file'], $post_id, $set_thumb);
	return $res;
}

function hhpanda_insert_attachment($file, $post_id, $set_thumb)
{
	$dirs 							= wp_upload_dir();
...
function hhpanda_get_list_episode($sourcePage, $post_id)
{
	# Xử lý episodes
	$server_add = array();
	if ($sourcePage["episodes"][0]["server_data"][0]["link_m3u8"] !== "") {
		foreach ($sourcePage["episodes"] as $key => $servers) {
			...
			foreach ($servers["server_data"] as $episode) {
				$slug_array 											= hhpanda_slugify($episode["name"], '_');
...
function hhpanda_slugify($str, $divider = '-')
{
...
function hhpanda_getStatus($status) {
	$hl_status = "completed";
