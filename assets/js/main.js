jQuery(function ($) {
  ...
  const crawl_page_callback = (currentPage) => {
    var urlPageCrawl = `https://sv.xemphimmoi.xyz/danh-sach/phim-moi-cap-nhat?page=${currentPage}`;
  ...
  // Crawler Schedule
  $("#save_crawl_hhpanda_schedule").on("click", () => {
    ...
    $.ajax({
      ...
      type: "POST",
      data: {
        action: "crawl_hhpanda_save_settings",
        pageFrom,
        pageTo,
        filterType,
        filterCategory,
  ...
  $("#crawl_hhpanda_schedule_enable").on("click", (e) => {
    let enable = $("#crawl_hhpanda_schedule_enable").is(":checked");
    $.ajax({
      url: ajaxurl,
      type: "POST",
      data: {
        action: "crawl_hhpanda_schedule_enable",
        enable
      },
      success: function (res) {
      },
  ...
  $("#save_crawl_hhpanda_schedule_secret").on("click", (e) => {
    let secret_key = $("input[name='crawl_hhpanda_schedule_secret']").val();
    $.ajax({
      url: ajaxurl,
      type: "POST",
      data: {
        action: "save_crawl_hhpanda_schedule_secret",
        secret_key
      },
      success: function (res) {
        alert("Lưu thành công!");
